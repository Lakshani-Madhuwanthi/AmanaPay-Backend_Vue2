      ...
      <div class="columns" style="width: 100%">
        <div class="column">
          <b-breadcrumb
              align="is-left"
          >
            <b-breadcrumb-item tag='router-link' to="" active >Banks</b-breadcrumb-item>
          </b-breadcrumb>
        </div>
        <div class="column">
          <b-button v-if="hasPermission('bank-create')" type="is-success is-small" class="is-pulled-right" icon-right="plus" @click="navigateTo('BankCreate')">Create Bank</b-button>
        </div>
      </div>
    </div>
    <div class="card-content" style="overflow: auto; height: 83vh">
      <change-request v-if="hasPermission('bank-approve')" :data="crData" :filter-out="filterOutFromCR" :is-loading="crIsLoading" @approve-or-reject="approve_or_reject" />
      ...
      <b-table
        ...
        <b-table-column field="bank_code" label="Bank Code" sortable v-slot="props">
         {{ props.row.bank_code }}
        </b-table-column>

        <b-table-column field="bank_name" label="Bank Name" sortable v-slot="props">
          <div style="width: 200px">{{ props.row.bank_name }}</div>
        </b-table-column>

<!--        <b-table-column field="address" label="Address" sortable v-slot="props">-->
<!--          <div style="width: 200px">{{ props.row.address }}</div>-->
<!--        </b-table-column>-->

<!--        <b-table-column field="country_code" label="Country" sortable v-slot="props">-->
<!--          {{ props.row.country_code }}-->
<!--        </b-table-column>-->

        <b-table-column field="branch" label="Image" v-slot="props">
          <div>
            <img :src="getImageUrl(props.row.logo)" style="height: 40px">
          </div>
        </b-table-column>
        ...
        <b-table-column field="action" label="Action" v-slot="props" sticky>
          <div v-if="props.row.locked === 0">
            <div v-if="hasPermission('bank-edit')" class="b-tooltips">
              <b-tooltip label="Edit" position="is-left" type="is-dark" style="margin-left: 10px">
                <b-button type="is-danger"
                          size="is-small"
                          @click="navigateTo('BankEdit', {id: props.row.id})"
...
export default {
  ...
  methods: {
    ...
    loadData: function (){
      ...
      NetworkManager.apiRequest('api/dutyfree/DFBank/list', {}, function (e){
        if(e.statusCode === 200){
          self.crData = e.data.changeRequests
          self.banks = e.data.banks
        }
    ...
    approve_or_reject: function (data){
      ...
      NetworkManager.apiRequest('api/dutyfree/DFBank/approve', data, function (e){
        if(e.statusCode === 200){
          const params = {
            type: 'is-success',
            message: 'Bank ' + data.actionTaken + ' successfully'
          }
          ...
          const params = {
            type: 'is-warning',
            message: 'Bank ' + data.actionTaken + ' failed!'
          }
          self.$buefy.toast.open(params)
        }
