<template>
  <section>
      ...
      <div class="columns" style="width: 100%">
        <div class="column">
          <b-breadcrumb align="is-left">
            <b-breadcrumb-item tag='router-link' to="" active>Bill Transactions</b-breadcrumb-item>
          </b-breadcrumb>
        </div>
        <div class="column">
        </div>
    ...
    <div class="card-content" style="overflow: auto; height: 83vh">
      ...
      <b-table class="my-table" :height="vhToPixels(55)" :data="filteredList" :paginated="tableOptions.isPaginated"
        ...
        <b-table-column field="biller_code" label="Biller Code" sortable v-slot="props" width="200">
          {{ props.row.biller_code }}
        </b-table-column>
        ...
        <b-table-column field="card_payment_time" label="Card Payment Time" sortable v-slot="props">
          <p class="expanded-col" v-if="(new Date(props.row.card_payment_time).getFullYear()) < 1000">-</p>
          <span v-else class="tag is-success expanded-col">{{ getDateTimeForDisplaying(props.row.card_payment_time)
          }}</span>
        </b-table-column>
                ...
                <section class="card-content">
                  <div class="content pt-auto">
                    ...
                    <b-field label="Beneficiary Received Amount (LKR)" custom-class="is-small">
                      <b-input :value="numberWithCommas(props.row.beneficiary_received_amount_lkr)" size="is-small"
                        readonly></b-input>
                    </b-field>
                  </div>
                </section>
        ...
        <b-table-column field="bill_payment_time" label="Bill Payment Time" sortable v-slot="props">
          <span class="tag is-success expanded-col">{{ getDateTimeForDisplaying(props.row.bill_payment_time) }}</span>
        </b-table-column>

        <b-table-column field="bill_payment_status" label="Bill Payment Status" sortable v-slot="props">
          <p class="expanded-col">{{ props.row.bill_payment_status }}</p>
        </b-table-column>

        <b-table-column field="bill_payment_description" label="Bill Payment Description" sortable v-slot="props">
          <p class="expanded-col" style="width: 200px">{{ props.row.bill_payment_description }}</p>
        </b-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      exportWizard: new TransactionExportWizard("Bill Transactions"),
  ...
  methods: {
    ...
    generateXlsx: function () {
      ...
      self.exportWizard.exportToXlsx(
        ...
        self.fromDate,
        self.toDate,
        "billTransactions_" + self.formatDateTime(new Date()),
        self.tableHeaders,
        self.filteredList
      )
    },
    ...
    loadData: function () {
      ...
      NetworkManager.apiRequest('api/Transaction/billTransactions', data, function (e) {
        if (e.statusCode === 200) {
