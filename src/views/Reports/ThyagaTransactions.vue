  ...
  <section>
      ...
      <div class="columns" style="width: 100%">
        <div class="column">
          <b-breadcrumb
              align="is-left"
          >
            <b-breadcrumb-item tag='router-link' to="" active >Thyaga Transactions</b-breadcrumb-item>
          </b-breadcrumb>
        </div>
        <div class="column">
        </div>
    ...
    <div class="card-content" style="overflow: auto; height: 83vh">
      ...
      <b-table
        ...
        <b-table-column field="mobile_number" label="Receiver Phone" sortable v-slot="props" width="200">
          {{ props.row.mobile_number }}
        </b-table-column>

        <b-table-column field="receiver_name" label="Receiver Name" sortable v-slot="props" width="200">
          {{ props.row.receiver_name }}
        </b-table-column>

        <b-table-column field="send_later" label="Send Later" sortable v-slot="props" width="200">
          {{ props.row.send_later ? 'Yes' : 'No' }}
        </b-table-column>

        <b-table-column field="send_at" label="Send At" sortable v-slot="props" width="200">
          <span class="tag is-success expanded-col">{{ getDateTimeForDisplaying(props.row.send_at) }}</span>
        </b-table-column>

        <b-table-column field="sender_name" label="Sender Name" sortable v-slot="props" width="200">
          {{ props.row.sender_name }}
        </b-table-column>

        <b-table-column field="via_email" label="Via Email" sortable v-slot="props" width="200">
          {{ props.row.via_email ? 'Yes' : 'No' }}
        </b-table-column>

        <b-table-column field="via_sms" label="Via SMS" sortable v-slot="props" width="200">
          {{ props.row.via_sms ? 'Yes' : 'No' }}
        </b-table-column>
        ...
        <b-table-column field="amount" :label="'Amount'" v-slot="props" sortable width="200">
        ...
        <b-table-column field="thyaga_payment_time" label="Thyaga Payment Time" sortable v-slot="props">
          <p class="expanded-col" v-if="(new Date(props.row.thyaga_payment_time).getFullYear()) < 1000">-</p>
          <span v-else class="tag is-success expanded-col">{{ new Date(props.row.thyaga_payment_time).toLocaleString() }}</span>
        </b-table-column>

        <b-table-column field="thyaga_payment_status" label="Thyaga Payment Status" sortable v-slot="props">
          <p class="expanded-col">{{ props.row.thyaga_payment_status }}</p>
        </b-table-column>

        <b-table-column field="thyaga_payment_description" label="Thyaga Payment Description" sortable v-slot="props">
          <p class="expanded-col" style="width: 200px">{{ props.row.thyaga_payment_description }}</p>
        </b-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      exportWizard: new TransactionExportWizard("Thyaga Transactions"),
  ...
  methods: {
    ...
    generateXlsx: function (){
      ...
      self.exportWizard.exportToXlsx(
          ...
          self.fromDate,
          self.toDate,
          "thyagaTransactions_" + self.formatDateTime(new Date()),
          self.tableHeaders,
          self.filteredList
      )
    },
    ...
    loadData: function (){
      ...
      NetworkManager.apiRequest('api/Transaction/thyagaTransactions', data, function (e){
        if(e.statusCode === 200){
